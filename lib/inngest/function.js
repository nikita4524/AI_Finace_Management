...
async function generateFinancialInsights(stats, month) {
  ...
  // Reuse a resilient resolver to handle model naming/API differences.
  ...
  let model;
  try {
    model = await resolveModel(GEMINI_MODEL, GEMINI_API_VERSION, process.env.GEMINI_FALLBACK_MODEL);
  } catch (mErr) {
    console.error("Error resolving generative model for insights:", mErr.message);
    throw new Error(
      `Configured model '${GEMINI_MODEL}' is not available for generating insights. Update GEMINI_MODEL or GEMINI_API_VERSION.`
    );
  }
